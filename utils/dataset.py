...
from preprocess.features_generators import get_features_generator
from preprocess.scaler import StandardScaler
...
def get_data(args, path: str, skip_invalid_smiles: bool = True) -> MoleculeDataset:
    ...
    with open(path) as f:
        ...
        task_names = header[1:]  # get task names (skip smiles column)
        lines = []
        for line in reader:
            smiles = line[0]
    ...
    return data, max_len, task_names
...
def mp_pool_map(list_input, func, num_workers):
    ...
    print(f"\n开始转换处理，数据量: {len(list_input)}, 进程数: {num_workers}")
    ...
    class _CollateFn(object):
        ...
        def __call__(self, data_list):
            ...
            for data in data_list:
                index, input_data = data
                try:
                    result = self.func(input_data)
                    new_data_list.append((index, result))
                except Exception as e:
                    print(f"处理数据 {index} 时出错: {str(e)}")
                    new_data_list.append((index, None))
            return new_data_list

    # 添加索引
    list_new_input = [(index, x) for index, x in enumerate(list_input)]

    # 创建数据加载器，添加进度条（替换原来的 pgl.Dataloader -> torch.utils.data.DataLoader）
    data_gen = TorchDataLoader(
        list_new_input,
        batch_size=8,
    ...
    for sub_outputs in tqdm(data_gen,
                            total=total_batches,
                            desc="转换处理",
                            ncols=100):
        list_output.extend(sub_outputs)
...
class InMemoryDataset(object):
    ...
    def _load_npz_data_path(self, npz_path):
        """加载NPZ文件数据"""
        try:
            # 如果是具体的npz文件路径
            if npz_path.endswith('.npz'):
                return load_npz_to_data_list(npz_path)
            ...
            # 如果是目录路径
                ...
                if not files:
                    raise ValueError(f"在 {data_path} 中没有找到.npz文件")

                files = sorted(files)  # 排序确保顺序一致
                data_list = []
                for f in files:
                    file_path = os.path.join(data_path, f)
                    data_list.extend(load_npz_to_data_list(file_path))
                return data_list

            else:
                raise ValueError(f"无效的路径: {npz_path}")
