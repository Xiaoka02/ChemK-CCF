...
from preprocess.data import MoleculeDataset
...
def index_select_ND(source: torch.Tensor, index: torch.Tensor) -> torch.Tensor:
    """
    Perform index selection on N-dimensional tensor.
    :param source: (N, M, ...) tensor
    :param index: (O, ...) tensor containing indices to select
    :return: (O, M, ...) tensor containing selected elements
    """
    index_size = index.size()  # (O, ...)
    suffix_dim = source.size()[1:]  # (M, ...)
    final_size = index_size + suffix_dim  # (O, M, ...)

    # Safety check
    max_idx = source.size(0)
    index = torch.clamp(index, 0, max_idx - 1)

    # Flatten indices
    flat_index = index.view(-1)

    # Execute index selection
    target = source.index_select(dim=0, index=flat_index)  # (O * ..., M, ...)
    target = target.view(final_size)  # (O, M, ...)

    # Create mask to avoid using expand_as
    mask = (index == 0)
    for _ in range(len(suffix_dim)):
        mask = mask.unsqueeze(-1)
    mask = mask.expand(final_size)

    # Use where operation instead of direct assignment
