...
# removed dependency on pgl (paddle-based library) to avoid requiring paddle.fluid
def md5_hash(string):
    """tbd"""
...
class GeoPredTransformFn(object):
    ...
    def __call__(self, raw_data):
        """
        Convert input data to molecule

        Args:
            raw_data: Can be SMILES string or dict containing SMILES

        Returns:
            Converted molecule data
        """
        # If input is dict, get SMILES
        if isinstance(raw_data, dict):
            smiles = raw_data.get('smiles')
            if smiles is None:
                print(f"Error: 'smiles' key not found in input dict: {raw_data}")
                return None
        else:
            smiles = raw_data
